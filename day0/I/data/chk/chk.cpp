...
int main(int argc, char **argv){
	...
	while(fscanf(ansFile,"%lf",&ans)==1)
	{
		int re=fscanf(outFile,"%lf",&out);
		if(re!=1)ret(0,"Output too short.");
		if(fabs(out-ans)<=1e-6);
			else ret(0,"Wrong Answer.");
	}
	if(fscanf(outFile,"%lf",&out)==1)ret(0,"Output too long.");
